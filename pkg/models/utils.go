...
// A constant for the Argon2ID prefix.
...
func (a Argon2ID) Verify(plain, hash string) (bool, error) {
	...
	if len(hashParts) < 6 {
		return false, fmt.Errorf("text not hashed")
	}

	// Check if the hash starts with the Argon2ID prefix.
	if hashParts[1] != argon2IDPrefix {
